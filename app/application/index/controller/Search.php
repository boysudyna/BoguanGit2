...
class Search extends Controller
{
    ...
    {	
        ...
        if(input('get.type') == 1){
        	...
        	foreach ($yearArr as $key => $value) {
    			...
    			if($prev) {
    				...
    				$per = round($ret/$allTotal, 4);
    				$abPer = round($abRet/$abnormalTotal, 4);
    		...
    		// DOU计算
        	$where = array();
        	$douArr = [100, 500, 1024, 5120, 10240, 40480];
        ...
        if(input('get.type') == 2){
            ...
            // file_put_contents($fileText, $str);
    		// 初始条件 - 网龄
    		$initYearArr = ['2003-04-01','2007-04-01', '2008-04-01','2009-04-01','2010-04-01','2011-04-01','2012-04-01','2013-04-01'];
        ...
        if(input('get.type') == 3){
            ...
            foreach($initYearArr as $key => $value) {
                ...
                for($i = 5; $i <= 7; $i++) {
                    ...
                    // 额外条件 - ARPU
                    $extWhere = array();
                    $arpuArr = [100,200,250,300,350,400];
                    foreach($arpuArr as $key => $value) {
                        $curr = $value;
                        $last = $arpuArr[$key+1] ? $arpuArr[$key+1] : '';
                        if($last) {
                    ...
                    // 额外条件 - DOU
                    $extWhere = array();
                    $douArr = [500, 1024, 5120, 10240, 40480];
                    foreach($douArr as $key => $value) {
                        $curr = $value;
                        $last = $douArr[$key+1] ? $douArr[$key+1] : '';
                        if($last) {
